...
Group-respecting permutations move label blocks between equal-sized independent groups (or shuffle within the sole block). Bootstrap intervals resample configured independent units. Learning curves should use increasing counts of independent groups; v0.1 exposes the resampling primitive but does not claim a curve when group coverage is too small.
