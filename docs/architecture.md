...
The vertical slice has one execution path. `CaseSpec` validates intent; a tabular adapter loads data; audits produce structured evidence; a shared non-scoring readiness decision blocks `ACTION_REQUIRED` findings before feature construction; split strategies persist checksummed index manifests; rows excluded by every manifest are omitted from run audits, features, predictions, and capability verdicts; leakage-safe pipelines fit bounded probes; evaluation emits test metrics and per-run predictions; stability and rule-based capability functions consume those artifacts; rendering produces Markdown, JSON, HTML, Parquet tables, and static figures.
