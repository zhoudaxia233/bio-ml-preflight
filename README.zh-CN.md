...
为研究者与助手审查生物数据提供可复核的计算证据：计数、划分检查、低成本基线和来源记录。

研究者与助手负责阅读论文、明确主张和实验单位、解释结果，以及判断还缺什么证据。
工具的能力状态只是已配置检查的汇总，不能代替这些科研判断，也不能证明论文结论。
具体分工见[证据审阅流程](docs/methodology.md#researcher-and-assistant-workflow)。

## 能做什么

- 检查缺失值、重复样本、重复实体、批次、重复测量和标签冲突
- 记录实体、重复对和可疑 ID 风险；根据已声明的验证目的区分必要重叠与划分冲突
- 比较随机、分组、冷实体、双冷实体、时间和自定义切分
- 运行 Dummy、线性模型、Extra Trees、梯度提升、近邻等 CPU 基线
- 在完全相同的切分清单和模型族下比较字符哈希与 RDKit Morgan 指纹
- 统计各类别的独立单位支持数，运行标签置换和学习曲线；尚不提供 Bootstrap 置信区间
...
这些发现还会汇总成一个不打分的建模前决定：没有检测到限制时为 `READY`；存在明确
警告或 `NOT_ASSESSABLE` 边界、但仍可建模时为 `READY_WITH_LIMITS`；出现必须先处理的
`ACTION_REQUIRED` 问题时为 `BLOCKED`。`run` 会在构造特征和拟合模型前执行同一门禁。
未确认的角色、目标或实体 ID 缺失、未解决的身份冲突、入模数值为无穷，以及把目标列
或结果后变量当特征，都会阻断运行。普通特征缺失仍是警告，因为插补可以限制在训练折
内。对于锁定 holdout，工具会先在访问数据前检查声明本身；访问账本登记后，依赖数据的
门禁只审计 supplied split 的训练行。对于完全由 supplied split 构成的评估，身份处理
策略同样不能使用被保留行的结果。二分类目标必须用数值 `0` 和 `1` 编码，以匹配当前的
指标与概率契约；回归和排序目标必须是至少包含两个有限取值的数值。每个被保留的评估
分区会在 holdout 访问登记之后、特征构造之前执行同一套可运行数据契约检查，其结果不会
被用于选择或删除行。被所有切分 manifest 排除的行不会进入运行审计、特征构造、预测或
能力结论。
