...
def test_bbb_martins_public_smoke(tmp_path: Path) -> None:
    ...
    rows = {row["claim_or_scenario"]: row for row in result["capability_matrix"]}
    assert metadata["row_count"] == len(data)
    assert rows["random_compound"]["status"] in {"SUPPORTED", "SUPPORTED_WITH_LIMITS"}
    assert rows["scaffold_generalization"]["status"] in {
        "SUPPORTED",
        "SUPPORTED_WITH_LIMITS",
        "INSUFFICIENT_EVIDENCE",
    }
    assert (
        rows["random_compound"]["numbers"]["median"]
        > rows["scaffold_generalization"]["numbers"]["median"]
    )
    assert rows["random_compound"]["numbers"]["permutation_draws"] == 9
